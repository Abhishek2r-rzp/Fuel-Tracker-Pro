...
export function categorizeTransaction(description, amount = null) {
  ...
  const hasIncomeKeyword = SYSTEM_CATEGORIES["Income"].keywords.some(
  ...
  Object.entries(SYSTEM_CATEGORIES).forEach(([category, data]) => {
    if (category === "Income" || category === "Others") return;
...
export function getCategoryColor(category) {
  return (
    SYSTEM_CATEGORIES[category]?.color || SYSTEM_CATEGORIES["Others"].color
  );
}
